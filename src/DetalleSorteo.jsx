...
export default function DetalleSorteo({ sorteos, setSorteos, isMobile }) {
  ...
  const UnirseAlSorteo = async (nombre, mail, facebook) => {
    try {
      ...
      if (peticion?.data?.success) {
        ...
        obtenerSorteos(); // recargar datos
      } else {
        // Muestra mensaje de error si el backend responde con error
        alert(peticion?.data?.error || 'No se pudo unir al sorteo.');
      }
    } catch (error) {
  ...
  const actualizarYoutubeDesdeComentario = (comentarios) => {
    ...
    // Convertir nombre de usuario a minúsculas para evitar errores por mayúsculas
    const nombre = usuarioKick.toLowerCase();
    ...
    if (match) {
      axios.post(`https://backmu.vercel.app/sorteo/${url}/actualizar-youtube`, {
        ...
        console.error('Error actualizando YouTube:', err);
      });
    }
  };
  ...
  const getUserColor = (messageCount) => {
    ...
    if (messageCount >= 45) return { color: '#33e64bcc', bonus: 4 }; // Verde Claro (+4%)
    if (messageCount >= 15) return { color: '#008a13cc', bonus: 2 }; // Verde Oscuro (+2%)
                            ...
                            <ListItemText
                              primary={
                                <span style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                  ...
                                  {u?.youtube && <FaYoutube style={{ marginLeft: '8px', color: 'red' }} />}
                            ...
                            <Grid style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                              ...
                              <Typography style={{ color }}> {ganador} {participante?.facebook && (
                                ...
                                {participante?.youtube && (
                                  <FaYoutube style={{ marginLeft: '8px', color: 'red' }} />
