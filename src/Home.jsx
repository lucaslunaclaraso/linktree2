...
const Home = (props) => {
  ...
  const [tipeos, setTipeos] = useState()
  ...
  useEffect(() => {
    ...
    const fetchSolicitudes = async () => {
      ...
      try {
        const response = await axios.get('https://backmu.vercel.app/solicitudes/aceptadas');
        setTipeos(response.data);
      } catch (err) {
    ...
    <Nlayout>
      <Grid style={{ background: '#11111d', marginTop: props.isMobile ? '-35%' : '-10%', width: '100%' }}>
        ...
        <Grid style={{ display: 'flex', flexDirection: 'column', height: '100vh' }} className="backgroundAnimado" onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
          ...
          <Grid style={{ width: '100%', position: 'relative' }} className="inner-grid">
          ...
          <Grid style={{ padding: 50, gap: '10px', zIndex: 9999 }} >
              ...
              <Box style={{ border: '1px solid #ffca41', borderRadius: '200px', padding: '8px', width: 'fit-content' }}>
                ...
                <Grid style={{ border: '1px solid #34375f', borderRadius: '200px', background: 'linear-gradient(96deg,transparent 34.16%,rgba(105,105,230,.2) 115.52%),linear-gradient(58deg,rgba(19,21,31,.75) -41.21%,rgba(39,43,65,.75) 161.22%)', padding: '40px  72px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <Typography sx={{
                    ...
                    color: 'white',
                    fontFamily: 'PoetsenOne'
                  }}>{(tipeos?.totalAceptadas * (2 * venta))?.toLocaleString('es-AR', {
        ...
        <Grid className={!props.isMobile && 'container'} style={{ padding: 50, gap: '10px', display: 'flex', flexDirection: 'column' }}>
          ...
          <Box style={{ border: '1px solid #830000', borderRadius: '200px', padding: '8px', width: 'fit-content', marginTop: '3%', margin: props.isMobile && '0 auto' }} className="pulse">
            <Link to="https://www.youtube.com/@dengueetimba" style={{ textDecoration: 'none' }}>
              <Button style={{ border: '1px solid #34375f', borderRadius: '200px', background: 'linear-gradient(0deg,rgba(0,0,0,.2),rgba(0,0,0,.2)),linear-gradient(122deg,#670000 -24.73%,red 53.1%,#830000 121.81%)', padding: '5px 25px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', gap: '10px' }} href="https://www.youtube.com/@dengueetimba" target='_blank' >
